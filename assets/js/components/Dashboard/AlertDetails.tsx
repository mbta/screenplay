...
const AlertDetails: ComponentType = () => {
  ...
  const [contextState, setStateFromContext] = useState(screenplayContext)
  ...
  useEffect(() => {
    const selectedAlert = screenplayContext.alerts.length ? screenplayContext.alerts.find((alert) => alert.id === id) : null;

    if (selectedAlert) {
      setStateFromContext(screenplayContext)
  ...
  return selectedAlert ? (
    <>
      <div className="alert-details">
        ...
        {Object.keys(screensByAlertMap).length !== 0 && <div className="page-content__body">
      ...
      <Modal className="alert-not-found" backdropClassName="alert-not-found" show={show} onHide={handleClose}>
        <Modal.Body>
          ...
          <div className="modal-text">
            ...
            {formatEffect(selectedAlert.effect)} #{selectedAlert.id} was just closed in Alerts UI.
              If this alert was previously showing on any screens, it has since been removed.
            </p>
            <Button className="screenplay-button modal-button" variant="primary" onClick={() => navigate("/alerts", { replace: true })}>
