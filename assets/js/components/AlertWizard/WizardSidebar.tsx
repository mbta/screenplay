...
import { svgFontSize, svgLandscapeLineLength, svgLineHeight, svgLongSide, svgPortraitLineLength, svgShortSide } from '../../constants/misc'
...
const createSVGtext = (message: string, x: number, y: number, orientation: string, maxCharsPerLine: number, lineHeight: number) => {
...
const WizardSidebar = (props: WizardSidebarProps): JSX.Element => {
  ...
  if (portraitSVG && props.step > 1) {
    const svgText = createSVGtext(props.message, svgShortSide/2, svgLongSide/2, 'portrait', svgPortraitLineLength, svgLineHeight)
    portraitSVG.append(svgText);
  }
  const landscapeSVG = document.getElementById('landscape-svg')
  if (landscapeSVG && props.step > 1) {
    const svgText = createSVGtext(props.message, svgLongSide/2, svgShortSide/2, 'landscape', svgLandscapeLineLength, svgLineHeight)
  ...
  return (
    <div className="wizard-sidebar">
      ...
      <svg className="portrait-svg"
          viewBox={"0 0 "+svgShortSide+" "+svgLongSide} height={svgLongSide} width={svgShortSide}>
        <svg id="portrait-svg" viewBox={"0 0 "+svgShortSide+" "+svgLongSide}>
          <rect stroke="black" fill="yellow" height="100%" width="100%"/>
          <text id="portrait-text"/>
        </svg>
      </svg>

      <svg className="landscape-hidden"
          viewBox={"0 0 "+svgLongSide+" "+svgLongSide} height={svgLongSide} width={svgLongSide}>
        <svg id="landscape-svg" viewBox={"0 0 "+svgLongSide+" "+svgShortSide}>
          <rect stroke="black" fill="yellow" height="100%" width="100%"/>
