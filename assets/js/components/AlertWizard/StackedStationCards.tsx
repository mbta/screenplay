...
const StackedStationCards = (props: StackedStationCardsProps): JSX.Element => {
  ...
  return (
    <div className={`stacked-station-cards ${props.className ? props.className : ''}`}>
      { props.selectedStations.map(station => {
        ...
        return (
          <React.Fragment key={station.name}>
            <div className={`station-card read-only ${props.className ? props.className : ''}`}>
              {lines.map(line => (
              ...
              {station.portrait && <div className="screen-icon portrait" />}            </div>
            <div className={`separator ${props.className ? props.className : ''}`}><div></div></div>
          </React.Fragment>
        )
      })}
    </div>
