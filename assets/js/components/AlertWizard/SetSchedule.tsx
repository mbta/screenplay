    ...
    <>
      <div className="step-instructions flex">
        <div className="hang-left">
          ...
          <div>Choose an expected play duration. After that, we’ll remind you to extend or clear the alert.</div>
        </div>
      </div>
      <div className="step-body">
        ...
        <div className="">
          <select className="message-select text-16" value={props.duration} onChange={props.changeDuration}>
            ...
            { Array(24).fill(0).map((_: number, index: number) => {
              const plural = index === 0 ? 'hour' : 'hours'
              return <option key={ index + 1 } value={ index + 1 }>{ index + 1 } { plural }</option>
            })}
          </select>
        </div>
      </div>
